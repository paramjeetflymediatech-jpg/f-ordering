...
export async function sendEmailReceipt(params: SimulatedEmailParams) {
  const { storeName, customer, order, payment, adminEmail, deviceInfo } = params;
...
Email: ${customer.email || 'Not Provided (Guest)'}
${order.deliveryAddress ? `Delivery Address: ${order.deliveryAddress}\n` : ''}
${deviceInfo ? `-------------------------- ORDER ORIGIN --------------------------------
IP Address: ${deviceInfo.ip || 'Unknown'}
Location: ${deviceInfo.address || (deviceInfo.geo ? [deviceInfo.geo.city, deviceInfo.geo.country].filter(Boolean).join(', ') : 'Unknown')}
...
`;
  ...
  const htmlBodyAdmin = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; color: #1e293b;">
      ...
      ${deviceInfo ? `
      <div style="margin-top: 20px; padding: 16px; background-color: #fff7ed; border-radius: 12px; border: 1px solid #fed7aa; font-size: 13px;">
        <h3 style="margin: 0 0 12px 0; font-size: 14px; text-transform: uppercase; color: #92400e; letter-spacing: 0.05em;">📍 Order Origin</h3>
        ${deviceInfo.ip ? `<p style="margin: 4px 0;"><strong>IP Address:</strong> <code style="background:#fef3c7;padding:1px 5px;border-radius:4px;">${deviceInfo.ip}</code></p>` : ''}
        ${deviceInfo.address ? `<p style="margin: 4px 0;"><strong>Full Address:</strong> ${deviceInfo.address}</p>` : (deviceInfo.geo?.city ? `<p style="margin: 4px 0;"><strong>Location:</strong> ${[deviceInfo.geo.city, deviceInfo.geo.country].filter(Boolean).join(', ')}</p>` : '')}
        ${deviceInfo.device ? `<p style="margin: 4px 0; word-break: break-all;"><strong>Device / Browser:</strong> <span style="color:#64748b;font-size:11px;">${deviceInfo.device}</span></p>` : ''}
        <p style="margin: 4px 0;"><strong>Placed At:</strong> ${timestamp}</p>
      </div>` : ''}
...
export async function sendBookingNotificationEmail(params: {
  ...
  const { storeName, adminEmails, customer, booking, deviceInfo } = params;
  const timestamp = new Date().toLocaleString();
...
`;
  ...
  const htmlBodyAdmin = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; color: #1e293b;">
      ...
      ${deviceInfo ? `
      <div style="margin-top: 20px; padding: 16px; background-color: #fff7ed; border-radius: 12px; border: 1px solid #fed7aa; font-size: 13px;">
        <h3 style="margin: 0 0 12px 0; font-size: 14px; text-transform: uppercase; color: #92400e; letter-spacing: 0.05em;">📍 Booking Origin</h3>
        ${deviceInfo.ip ? `<p style="margin: 4px 0;"><strong>IP Address:</strong> <code style="background:#fef3c7;padding:1px 5px;border-radius:4px;">${deviceInfo.ip}</code></p>` : ''}
        ${deviceInfo.address ? `<p style="margin: 4px 0;"><strong>Full Address:</strong> ${deviceInfo.address}</p>` : (deviceInfo.geo?.city ? `<p style="margin: 4px 0;"><strong>Location:</strong> ${[deviceInfo.geo.city, deviceInfo.geo.country].filter(Boolean).join(', ')}</p>` : '')}
        ${deviceInfo.device ? `<p style="margin: 4px 0; word-break: break-all;"><strong>Device / Browser:</strong> <span style="color:#64748b;font-size:11px;">${deviceInfo.device}</span></p>` : ''}
        <p style="margin: 4px 0;"><strong>Booked At:</strong> ${timestamp}</p>
      </div>` : ''}
